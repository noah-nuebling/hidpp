...
struct RawDevice::PrivateImpl
{
    ...
    std::queue<std::vector<uint8_t>> inputReportQueue; // Can't make this atomic, use explicit mutexes to protect this instead
    ...
    static void initState(RawDevice *dev) {
        ...
        dev->_p->inputReportQueue = std::queue<std::vector<uint8_t>>();
    ...
    // input report buffer
    //         This is (probably) purely for IOHIDDeviceRegisterInputReportCallback(), we don't use it directly 
...
RawDevice::RawDevice(const std::string &path) : _p(std::make_unique<PrivateImpl>()) {
    ...
    // Setup input reportBuffer
    CFIndex reportBufferSize = _p->maxInputReportSize;
    _p->reportBuffer = (uint8_t *) malloc(reportBufferSize);
    memset(_p->reportBuffer, 0, reportBufferSize); // Init with 0s

    // Setup device report callback
    ...
    IOHIDDeviceRegisterInputReportCallback(
        _p->iohidDevice, 
        _p->reportBuffer, // This will get filled when an inputReport occurs
        ...
        [] (void *context, IOReturn result, void *sender, IOHIDReportType type, uint32_t reportID, uint8_t *report, CFIndex reportLength) {
            ...
            // Lock
            //      Not necessary since this runs synchronously on readReport()'s thread now
            ...
            // Store report in queue
            devvv->_p->inputReportQueue.push(std::vector(report, report + reportLength));
...
int RawDevice::writeReport(const std::vector<uint8_t> &report)
{
    ...
    //  TODO: Does this really make sense? Also shouldn't we delete the inputReportQueue?
...
const char *CFRunLoopRunResult_ToString(CFRunLoopRunResult runLoopResult) {
    
    const char * result;
    switch (runLoopResult) {
        break; case kCFRunLoopRunFinished:         result = "Finished";
        break; case kCFRunLoopRunHandledSource:    result = "HandledSource";
        break; case kCFRunLoopRunStopped:          result = "Stopped";
        break; case kCFRunLoopRunTimedOut:         result = "TimedOut";
        break; default:                            result = "UnknownResult";
    };
...
int RawDevice::readReport(std::vector<uint8_t> &report, int timeout) {
    ...
    if (_p->inputReportQueue.empty()) { // Listen for input if there are no reports already queued up.
        ...
        // Define constant
        if ((0)) double lookbackThreshold = 1 / 100.0; // If a report occured no more than `lookbackThreshold` seconds ago, then we use it.
        ...
        bool onlyHandleOneSource = true;

        // Make device deliver events to runLoop we'll run on this thread.
        _p->inputReportRunLoop = CFRunLoopGetCurrent();
        IOHIDDeviceScheduleWithRunLoop(_p->iohidDevice, _p->inputReportRunLoop, runLoopMode);

        // Run the loop
        //  Will wait for inputReport and then fill up _p->inputReportQueue
        //  We only wanna stop waiting if one of these happens
        //      - Device sends input report
        //      - Timeout happens
        //      - interruptRead() is called
        CFRunLoopRunResult runLoopResult = CFRunLoopRunInMode(runLoopMode, timeoutSeconds, onlyHandleOneSource); 

        // Analyze runLoop exit reason
        Log::debug() << "inputReportRunLoop exited with result: " << CFRunLoopRunResult_ToString(runLoopResult) << std::endl;
    }

    int returnValue = 0;
    if (!_p->inputReportQueue.empty()) {
        std::vector<uint8_t> r = _p->inputReportQueue.front();
        _p->inputReportQueue.pop();
        report = r;
        returnValue = r.size();
    }
...
void oldStuff(void) {
    ...
    // Old code that ran on the runloop thread
